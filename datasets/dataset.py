...
class Dataset(object):
    ...
    def __save(self, output, images, A, A_inv, x_2d, x_3d):
        # check the size of arrays
        if not(len(images) == len(A) == len(A_inv) == len(x_2d) == len(x_3d)):
            raise AttributeError("The length of attributes is not the same, images({0}), A({1}), A_inv({2}), x_2d({3}) and x_3d({4}).".format(len(images), len(A), len(A_inv), len(x_2d), len(x_3d)))
        # output the file
        for images_i, A_i, A_inv_i, x_2d_t, x_3d_t  in zip(images, A, A_inv, x_2d, x_3d):
            x_i = np.vstack([np.vstack((x_2d_t[name], x_3d_t[name])) for name in self.JOINTS])
            output.write(",".join(map(str, [images_i] + list(chain.from_iterable(A_i.tolist())) + list(chain.from_iterable(A_inv_i.tolist())) + list(chain.from_iterable(x_i.tolist())))) + os.linesep)
        output.write(os.linesep)
    ## save the train/test dataset
    # @param self The object pointer
    def _saveDataset(self):
        for i in (0, 1):
            self.__save(self.__file[i], self._images[i], self._A[i], self._A_inv[i], self._x_2d[i], self._x_3d[i])
